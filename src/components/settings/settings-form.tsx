...
function BillingTab() {
  ...
  const usage = selectedOrganization?.usage || { executionsThisMonth: 0, billingCycleStart: new Date().toISOString() };

  // Calculate trigger count
  const triggerCount = (selectedOrganization?.triggers?.length || 0) +
    (selectedOrganization?.folders?.reduce((acc, f) => acc + (f.triggers?.length || 0), 0) || 0);
    ...
    <div className="space-y-6">
      <Card className="shadow-lg">
        <CardHeader>
          ...
          <CardDescription>You are currently on the <span className="capitalize font-bold text-primary">{planId}</span> plan.</CardDescription>
        </CardHeader>
        <CardContent className="space-y-8">
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Monthly Executions</span>
              <span className="font-medium">{usage.executionsThisMonth} / {limits.executionsPerMonth === Infinity ? '∞' : limits.executionsPerMonth}</span>
            </div>
            <Progress value={limits.executionsPerMonth === Infinity ? 0 : (usage.executionsThisMonth / limits.executionsPerMonth) * 100} className="h-2" />
            <p className="text-xs text-muted-foreground">Resets on {usage.billingCycleStart ? new Date(usage.billingCycleStart).toLocaleDateString() : 'next cycle'}</p>
          </div>

          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Active Triggers</span>
              <span className="font-medium">{triggerCount} / {limits.triggers === Infinity ? '∞' : limits.triggers}</span>
            </div>
            <Progress value={limits.triggers === Infinity ? 0 : (triggerCount / limits.triggers) * 100} className="h-2" />
          </div>

          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Minimum Interval</span>
              <span className="font-medium">{limits.minIntervalMinutes} minutes</span>
            </div>
          </div>
        </CardContent>
        <CardFooter>
